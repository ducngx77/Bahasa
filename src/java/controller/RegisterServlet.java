                ...
                if (exist == false) {
                    ...
                    switch (control) {
                        ...
                        case 2:
                            ...
                            break;
                        default:
                            String err = "Sai tên tài khoản hoặc mật khẩu!";
                            request.setAttribute("err", err);
                            request.getRequestDispatcher("login.jsp").forward(request, response);
                            break;
                    }
                } else {
                    String aerr = "Tài khoản hoặc email đã tồn tại!";
                    request.setAttribute("aerr", aerr);
                    request.getRequestDispatcher("register.jsp").forward(request, response);
                }
            } else {
                String perr = "Mật khẩu xác nhận không trùng khớp!";
                request.setAttribute("perr", perr);
                request.getRequestDispatcher("register.jsp").forward(request, response);
            }
        } else {
            String eerr = "Email không đúng định dạng!";
            request.setAttribute("eerr", eerr);
            request.getRequestDispatcher("register.jsp").forward(request, response);
        }
    }
