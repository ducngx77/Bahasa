                        ...
                        <table class="table ">
                            ...
                            <c:forEach items="${requestScope.listCustomer}" var="lc" varStatus="status"> 
                                <tr class="cross2 row">
                                    ...
                                    <td class="t-data ring-in col-md-4"><h3>${lc.name} | ${lc.username} | ID: ${lc.id}</h3>
                                        <div class="clearfix"> </div>
                                    </td>                                               
                                    <td class="t-data col-md-1"><h3>${lc.email}</h3></td>
                                    <td class="t-data col-md-1"><h3>${lc.phone}</h3></td> 
                                    <td class="t-data col-md-1"><h3>${lc.address}</h3></td>
                                    <td class="t-data col-md-4">
                                        <div class="col-md-6">
                                            <div>
                                                <input type="hidden" value="${lc.id}" name="offdfid${lc.id}">
                                                <h3><input id="bt${lc.id}" type="button" style="background-color: #4CAF50; color: white; border: none" value="Trở thành nhân viên" onclick="confirmUpgrade('${lc.id}')"> </h3>                                  
                                            </div>
                                            <script>
                                                function confirmUpgrade(id) {
                                                    var result = confirm("Xác nhận biến tài khoản này thành tài khoản nhân viên?");
                                                    if (result) {
                                                        window.location = "<%=request.getContextPath()%>/update?aid=" + id;
                                                    }
                                                }
                                            </script>
                                        </div>                              
